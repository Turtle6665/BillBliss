...
function checkBalance(debiters, crediters) {
    ...
    if (Math.abs(sumCrediters - sumDebiters) >= 0.01) {
        throw new Error(`Unsolvable : debiters (-${sumDebiters}) and crediters (+${sumCrediters}) are unbalanced.`);
    }
}
...
function reduceBalance(crediters, debiters, results = []) {
    ...
    crediters = crediters.sort((a, b) => {return a[1] - b[1] >= 0});
    debiters  = debiters.sort( (a, b) => {return b[1] - a[1] >= 0});
